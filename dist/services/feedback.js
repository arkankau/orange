 ...
 */
async function generateInterviewFeedback(transcript, bodyLanguage, questionIndex) {
    try {
        // Check API key
 ...
 */
function generateFallbackFeedback(transcript, bodyLanguage, questionIndex) {
    const transcriptLength = transcript.trim().length;
    const avgBodyLanguage = (bodyLanguage.warmth +
        bodyLanguage.competence +
        bodyLanguage.affect +
        (bodyLanguage.eyeContactRatio ?? 0.5) +
        (bodyLanguage.gestureIntensity ?? 0.5) +
        (bodyLanguage.postureStability ?? 0.5)) / 6;
