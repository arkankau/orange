...
import { BodyLanguageFeatures, GradingResult, MindmapAnalysis } from '../types/shared';
...
`;
  ...
  // Enhanced prompt from claude branch - includes mental model analysis
  const prompt = `You are an expert consulting interview coach grading a case interview response.
...
- Your model structure: ${JSON.stringify(mindmap.your_model.tree, null, 2)}
- Ideal model structure: ${JSON.stringify(mindmap.ideal_model.tree, null, 2)}
...
1. **Structure Score**: How well-structured and organized was the answer? Did they use a framework effectively? Consider how well they followed the ideal mental model structure.
2. **Insight Score**: How insightful and analytical was the answer? Did they identify key issues and provide good analysis? Consider the depth of their thinking.
3. **Communication Score**: How clear and effective was the communication? Was it easy to follow? Consider clarity, pacing, and articulation.

Also provide:
- **Body Language Score** (0-100): Based on the body language metrics provided
- **Comments**: 3-5 specific, actionable comments about what was good and what to improve. Reference the mental model gaps identified.
...
Return your response in this JSON format:
{
  ...
  "comments": [
    ...
    "Missing key concepts: [reference missing concepts]",
    "Work on prioritizing the most important factors first"
  ]
}
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      return {
        ...
        bodyLanguageScore: grading.bodyLanguageScore || Math.round(
          ((bodyLanguage.warmth + bodyLanguage.competence + (bodyLanguage.eyeContactRatio || 0.5)) / 3) * 100
        ),
...
function generateFallbackGrading(
  ...
  // Structure scoring
  if (transcriptLength > 100) structureScore += 15;
  if (transcript.toLowerCase().includes('first') || transcript.toLowerCase().includes('second')) structureScore += 10;
  if (transcript.toLowerCase().includes('framework') || transcript.toLowerCase().includes('structure')) structureScore += 10;

  // Insight scoring
  if (transcriptLength > 150) insightScore += 10;
  if (transcript.toLowerCase().includes('because') || transcript.toLowerCase().includes('reason')) insightScore += 10;
  if (transcript.toLowerCase().includes('analyze') || transcript.toLowerCase().includes('consider')) insightScore += 10;

  // Communication scoring
  if (transcriptLength > 80) communicationScore += 10;
  if (transcript.split('.').length > 3) communicationScore += 10; // Multiple sentences
